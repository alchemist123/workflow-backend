"""The workflow graph for Sub Agents.
...
Canvas node -> graph node:
    ...
    n_end_1                  END  (canvas 'end' — End)
    n_orchestrator_agent_2   ORCHESTRATOR_AGENT  (canvas 'orchestrator' — Orchestrator)
"""
...
n_orchestrator_agent_2 = NODES["n_orchestrator_agent_2"]

# ── Graph ─────────────────────────────────────────────────────────────────────
# AGENT_NAME must be a valid Python identifier: ADK validates a Workflow's name
# as a node name. The generator defaults it to "sub_agents".
root_agent = Workflow(
    name=settings.AGENT_NAME,
    description=settings.AGENT_DESCRIPTION,
    edges=[
        (START, a2a_start),
        (a2a_start, n_orchestrator_agent_2),
        (n_orchestrator_agent_2, n_end_1),
    ],
)
