"""The workflow graph for Human Approval.
...
Canvas node -> graph node:
    ...
    n_condition_6            CONDITION  (canvas 'size' — Needs approval?)
    n_end_3                  END  (canvas 'end' — End)
    n_human_approval_4       HUMAN_APPROVAL  (canvas 'gate' — Finance Sign-off)
    n_transform_1            TRANSFORM  (canvas 'auto' — Auto-approve)
    n_transform_2            TRANSFORM  (canvas 'declined' — Record Decline)
    n_transform_5            TRANSFORM  (canvas 'paid' — Record Payment)
"""
...
n_human_approval_4 = NODES["n_human_approval_4"]
...
# as a node name. The generator defaults it to "human_approval".
root_agent = Workflow(
    name=settings.AGENT_NAME,
    description=settings.AGENT_DESCRIPTION,
    edges=[
        (START, a2a_start),
        (a2a_start, n_condition_6),
        Edge(from_node=n_condition_6, to_node=n_human_approval_4, route=["large", "true"]),
        Edge(from_node=n_condition_6, to_node=n_transform_1, route=["small", "false"]),
        Edge(from_node=n_human_approval_4, to_node=n_transform_5, route="approved"),
        Edge(from_node=n_human_approval_4, to_node=n_transform_2, route="rejected"),
        (n_transform_1, n_end_3),
        (n_transform_2, n_end_3),
        (n_transform_5, n_end_3),
    ],
)
