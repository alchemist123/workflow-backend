...
byte for byte, rather than a separate execution path.
    ...
    python run_once.py --task <task-id>        # look a past task up
...
def get_task(task_id: str) -> dict:
    """Look one task up by id, without running or resuming anything.

    A plain `tasks/get`, which is exactly what any other A2A caller would send.
    It can answer at all because the task store is on disk beside the package
    (`STATE_DSN`) rather than in the memory of the process that created it — so
    a task submitted by an earlier invocation, or by a real caller against the
    deployed agent, is still there to look at.

    Read-only on purpose: a workflow parked on a human node stays parked. Use
    `--resume` to answer it.
    """
    from starlette.testclient import TestClient
    ...
    try:
        with TestClient(app, headers=_auth_headers()) as client:
            reply = _rpc(client, "tasks/get", {"id": task_id})
    except Exception as exc:  # noqa: BLE001
        return {
            "ok": False,
            "state": "failed",
    ...
    if reply.get("error"):
        # An unknown id is the ordinary case, not a crash: the task may belong
        # to a different workflow, or have been created before this package was
        # last re-rendered.
    ...
    envelope = {
        ...
        "result": result,
        "artifact_text": text,
        "error": None if state in ("completed", "input-required") else (
            f"task ended in state {state!r}" if state else "task has no state"
        ),
...
def main() -> int:
    ...
    # Looking a task up without touching it.
    parser.add_argument(
        "--task",
        metavar="TASK_ID",
        help="look one task up (tasks/get) and print its state; changes nothing",
    )
