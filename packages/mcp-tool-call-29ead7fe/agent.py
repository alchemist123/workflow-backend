"""The workflow graph for MCP Tool Call.
...
Canvas node -> graph node:
    ...
    n_end_1                  END  (canvas 'done' — Done)
    n_mcp_tool_4             MCP_TOOL  (canvas 'quote' — Price Quote)
    n_transform_2            TRANSFORM  (canvas 'invoice' — Invoice Line)
    n_transform_3            TRANSFORM  (canvas 'normalise' — Normalise)
"""
...
n_mcp_tool_4 = NODES["n_mcp_tool_4"]
...
# as a node name. The generator defaults it to "mcp_tool_call".
root_agent = Workflow(
    name=settings.AGENT_NAME,
    description=settings.AGENT_DESCRIPTION,
    edges=[
        (START, a2a_start),
        (a2a_start, n_transform_3),
        (n_mcp_tool_4, n_transform_2),
        (n_transform_2, n_end_1),
        (n_transform_3, n_mcp_tool_4),
    ],
)
