...
Canvas node -> graph node:
    ...
    n_parallel_fork_4        PARALLEL_FORK  (canvas 'fork' — Split)
    n_transform_1            TRANSFORM  (canvas 'checked' — Check Status)
    n_transform_6            TRANSFORM  (canvas 'sent' — Send Receipt)
    n_wait_2                 WAIT  (canvas 'cooloff' — Rate limit)
    n_wait_7                 WAIT  (canvas 'settle' — Let it settle)
"""
...
n_wait_2 = NODES["n_wait_2"]
n_wait_7 = NODES["n_wait_7"]

# ── Graph ─────────────────────────────────────────────────────────────────────
# AGENT_NAME must be a valid Python identifier: ADK validates a Workflow's name
# as a node name. The generator defaults it to "wait".
root_agent = Workflow(
    ...
    edges=[
        ...
        (n_merge_5, n_end_3),
        (n_parallel_fork_4, n_wait_7),
        (n_parallel_fork_4, n_wait_2),
        (n_transform_1, n_merge_5),
        (n_transform_6, n_merge_5),
        (n_wait_2, n_transform_6),
        (n_wait_7, n_transform_1),
    ],
)
