...
Generated for MCP Tool Call. Runs against the real application built
...
This workflow calls out to services configured in `.env`
(MCP tool servers), which are not reachable in an offline test run. The
run-completion tests therefore assert that the task reaches a *terminal* state
rather than `completed` — enough to catch the failure that matters, a task stuck
at `working` forever with no artifacts. With those services running and `.env`
filled in, they also pass on `completed`.
"""
...
EXAMPLE_PAYLOAD: dict = {'item': '', 'count': 0}
...
def test_blocking_message_send_completes(a2a_client):
    ...
    # Terminal, not necessarily completed: this workflow's dependencies are not
    # reachable offline. What must never happen is the task hanging.
    assert task["status"]["state"] in _TERMINAL, task["status"]
    if task["status"]["state"] == "completed":
        assert isinstance(json.loads(_artifact_text(task)), dict)
...
def test_both_modes_agree_on_the_result(a2a_client):
    ...
    # Offline, both modes fail the same way; the point is that they agree.
    assert blocking["status"]["state"] == polled["status"]["state"]
