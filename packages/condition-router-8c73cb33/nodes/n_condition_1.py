...
Route: score > 50 → premium, else → standard

Generated from canvas node 'condition'. Do not edit by hand:
...
BRANCHES: list[str] = ['high_score', 'low_score']
DEFAULT_ROUTE = None

log = node_logger(NODE_ID)


def _evaluate(data: dict, vars: dict) -> str | None:  # noqa: A002 - the canvas name
    """The canvas branch expressions, in order. First match wins.

    `data` is the payload from the previous node; `vars` is every named
    variable, so a branch can route on something an earlier node saved.
    """
    # high_score: data.get('score', 0) > 50
    try:
        if data.get('score', 0) > 50:
            return "high_score"
    except Exception:  # noqa: BLE001 - a branch that cannot be evaluated is not a match
        log.warning("branch %r could not be evaluated", "high_score")
    # low_score: data.get('score', 0) <= 50
    try:
        if data.get('score', 0) <= 50:
            return "low_score"
    except Exception:  # noqa: BLE001 - a branch that cannot be evaluated is not a match
        log.warning("branch %r could not be evaluated", "low_score")
    return None


@flow_node(name=NODE_ID, variable=OUTPUT_VARIABLE, **NODE_KWARGS(timeout=30))
async def n_condition_1(ctx: Context, node_input=None):
    data = node_input or {}
    branch = _evaluate(data, all_variables(ctx)) or DEFAULT_ROUTE
    if branch is None:
