"""HUMAN_APPROVAL — Finance Sign-off.

Generated from canvas node 'gate'. Do not edit by hand:
...
from nodes.base import NODE_KWARGS, flow_node, route_event
NODE_ID = "n_human_approval_5"
...
ASSIGNEES: list = ['finance@example.com']
# Advertised on the paused task so a client can build a form from it rather
# than guessing what to send back.
RESPONSE_SCHEMA: dict = {'type': 'object', 'properties': {'approved': {'type': 'boolean', 'description': 'True to approve, false to reject.'}, 'comment': {'type': 'string', 'description': 'Optional note explaining the decision.'}, 'cost_centre': {'type': 'string', 'description': 'Which cost centre to charge. Required when approving.'}}, 'required': ['approved']}
...
REQUIRED_FIELDS: list = ['cost_centre']
...
def _interrupt_id(ctx: Context) -> str:
    ...
    """
    return f"approval:{ctx.node_path}"
...
@flow_node(name=NODE_ID, variable=OUTPUT_VARIABLE, rerun_on_resume=True, **NODE_KWARGS(timeout=120))
async def n_human_approval_5(ctx: Context, node_input=None):
    data = node_input or {}
    ...
    if answer is None:
        log.info("waiting for a human decision (%s)", interrupt_id)
    ...
    if approved:
        ...
        if missing:
            # Ask again rather than approving on incomplete information.
            # Reusing the interrupt id is supported: ADK matches calls and
            # responses by count.
            log.info("approval is missing %s; asking again", ", ".join(missing))
            return _ask(
                f"{PROMPT or DEFAULT_PROMPT} "
                f"(still needed to approve: {', '.join(missing)})"
            )

    decision = {key: value for key, value in answer.items() if key != "approved"}
    decision["approved"] = approved

    log.info("human %s (%s)", "approved" if approved else "rejected", interrupt_id)
    merge_state(ctx, decision)
    return route_event(
        APPROVED_ROUTE if approved else REJECTED_ROUTE, {**data, **decision}
    )
