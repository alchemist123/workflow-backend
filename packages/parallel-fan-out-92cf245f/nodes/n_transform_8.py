...
Generated from canvas node 'total'. Do not edit by hand:
...
def _transform(data: dict, vars: dict) -> dict:  # noqa: A002 - `vars` is the canvas name
    ...
    """
    result = None
    result = {'total': round((vars['order']['amount'] or 0) + (data.get('tax') or 0) + (data.get('shipping') or 0) - (data.get('discount') or 0), 2)}
...
async def n_transform_8(ctx: Context, node_input=None):
    data = node_input or {}
    try:
